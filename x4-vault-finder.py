...
def maybe_store_object(path):
    ...
    elif is_station(path) or is_gate(path) or is_vault(path) or is_abandoned_ship(path):
        code = attrib['code']
...
def maybe_store_position(path):
    if not (is_station(path) or is_gate(path) or is_vault(path) or is_abandoned_ship(path)):
        return
    position = [0, 0, 0]
    for elem in path:
...
def is_abandoned_ship(path):
    return path[-1].tag == 'component' and path[-1].attrib.get('class', '').startswith('ship_') and path[-1].attrib.get("owner") == "ownerless"
